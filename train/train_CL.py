...
class Baseline_Trainer():
    def __init__(self, models, optimizers, schedulers,
        ...
        # add buffer
        self.buffer = Buffer(buffer_size = 2000, device = 'cuda')
    ...
    def train(self, epoch):
        ...
        for batch_idx, (inputs, targets) in enumerate(batch_iter):
            ...
            for m in self.models:
                if self.plus_adv:
                    temp = Linf_PGD(m, inputs, targets, **self.attack_cfg)
            ...
            for i, m in enumerate(self.models):
                ...
                if self.plus_adv:
                    # print('add adv examples')
                    real_batch_size = inputs.shape[0]
                    adv_inputs = adv_inputs_list[i]
                    if not self.buffer.is_empty():
                        buf_inputs, buf_labels = self.buffer.get_data(
                            real_batch_size, transform=torchvision.transforms.Compose([]))
                        adv_inputs = torch.cat((adv_inputs_list[i], buf_inputs))
                        targets = torch.cat((targets, buf_labels))

                    outputs = m(adv_inputs)
                    loss = self.coeff * loss + self.criterion(outputs, targets)

                    self.buffer.add_data(examples=adv_inputs_list[i],
                        labels=targets[:real_batch_size])
    ...
    def save(self, epoch):
        ...
        if epoch % 50 == 0:
            torch.save(state_dict, os.path.join(self.save_root, 'res_vanilla_cifa10_epoch_%d.pth'%epoch))
...
def main():
    ...
    save_root = os.path.join('checkpoints_advtrain_CL', 'baseline_c10', 'seed_{:d}'.format(args.seed), '{:d}_{:s}{:d}'.format(args.model_num, args.arch, args.depth))
